...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const fileType of fileTypes) {
      const folderPath = [auth.user.id, String(courseId), String(week.id), fileType].join("/");
      ...
      const candidates = sortNewestFirst(
        ((entries as StorageListEntry[] | null) ?? []).filter((entry) => entry.name && entry.name !== ".emptyFolderPlaceholder"),
      );
    ...
    if (importedMaterials.length === 0) {
      return NextResponse.json(
        { error: "Bu hafta için Supabase Storage içinde bağlanacak dosya bulunamadı." },
        { status: 404 },
      );
    }
